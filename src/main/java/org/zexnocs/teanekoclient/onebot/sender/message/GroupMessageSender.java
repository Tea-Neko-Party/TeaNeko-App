...
public class GroupMessageSender extends AbstractOnebotSender<GroupMsgSendParamsData, OnebotMessageResponseData> {
    ...
    /**
     * 获取一个 {@link GroupEasyMessageSenderBuilder}，用于构建一般 message 信息并发送。
     *
     * @param data  要回复的消息数据
     * @param token 发送器的 token，用于识别发送环境
     */
    public GroupEasyMessageSenderBuilder getBuilder(ITeaNekoMessageData data, String token) {
    ...
    public class GroupEasyMessageSenderBuilder implements IEasyMessageSenderBuilder {
        ...
        public TaskFuture<? extends IMessageResponseData> sendWithFuture() {
            ...
            var future = GroupMessageSender.this.sendMessage(
                    token,
                    messages,
                    repliedData.getUserData().getGroupId(),
                    delay,
                    retryCount,
                    retryInterval
            );
