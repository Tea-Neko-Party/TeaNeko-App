...
| `TeaNekoAgentInternalClient` | 在 App 进程内执行结构化 Agent send data，不建立 WebSocket。 |
...
该流程中的 sender 不是外部平台消息发送器。它是 App 内部调用 Agent 能力的标准入口，复用 `SenderService -> SentEvent -> IClient -> ResponseEvent -> ResponseService` 链路。
...
`TeaNekoAgentClient` 是 App 内置客户端，而不是 WebSocket 客户端。它只借用 App 的通用客户端和 sender/response 协议，Agent 专属实现全部位于 `org.zexnocs.teanekoagent`。
...
模型配置读取位于 `teanekoagent.llm.file_config`，Agent 主配置位于 `teanekoagent.file_config`，人格文件配置位于 `teanekoagent.personality.file_config`。
